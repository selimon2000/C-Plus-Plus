...
TEST(QuadcopterReachGoals, TwoGoals)
{
    ...
    {
        Odometry odo1 = populateOdoUGV(20, 20, 0);
        linkCommand->writeCommand(odo1);
    ...
    goals.push_back(pfms::geometry_msgs::Point{10, -2});
    goals.push_back(pfms::geometry_msgs::Point{10, 6});
    goals.push_back(pfms::geometry_msgs::Point{0, -2});
    goals.push_back(pfms::geometry_msgs::Point{-3, -20});
    goals.push_back(pfms::geometry_msgs::Point{-10, -20});
    goals.push_back(pfms::geometry_msgs::Point{-10, 40});
    goals.push_back(pfms::geometry_msgs::Point{-17, 40});
    goals.push_back(pfms::geometry_msgs::Point{-20, 9});
    goals.push_back(pfms::geometry_msgs::Point{-15, 5});
    goals.push_back(pfms::geometry_msgs::Point{0, 2});
    ...
    double distance = pow(pow(odo.position.x - goals.back().x, 2) +
                              pow(odo.position.y - goals.back().y, 2),
                          0.5);
    // double distance = pow(pow(odo.position.x - goals.at(0).x, 2) +
    //                           pow(odo.position.y - goals.at(0).y, 2),
    //                       0.5);
